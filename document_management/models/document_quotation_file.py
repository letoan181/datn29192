...
class DocumentQuotationFile(models.Model):
    _name = "document.quotation.file"
    ...
    def _inverse_is_favorite(self):
        favorite_document = not_fav_document = self.env['document.quotation.file'].sudo()
        for document in self:
    ...
    write_users = fields.Many2many('res.users', 'document_quotation_file_write_user_rel', 'res_file_id',
                                   'res_user_id',
                                   string='Write Users', default=lambda self: self.env.user)
    read_users = fields.Many2many('res.users', 'document_quotation_file_read_user_rel', 'res_file_id',
                                  'res_user_id',
                                  string='Read Users')
    favorite_user_ids = fields.Many2many(
        'res.users', 'document_quo_file_favorite_user_rel', 'document_id', 'user_id',
    ...
    def check_user_can_see_file(self):
        for rec in self:
            if len(rec.users_update) > 0:
                warning = []
                #users_can_see = [res.res_user_id for res in self.env['document.permission'].sudo().search([('res_id', '=', rec.res_id), ('model', '=', 'quotation')])]
                users_can_see = []
    ...
    @api.model
    def action_document_quotation_file_list(self):
        ...
        if self.user_has_groups('base.group_system'):
            ...
            self.env.cr.execute(
                """select id
from document_quotation_file
where id in (select res_file_id from document_quotation_file_write_user_rel where res_user_id = %s)
   or id in (select res_file_id from document_quotation_file_read_user_rel where res_user_id = %s)""",
                (self._uid, self._uid,))
            ...
            if can_read_documents is not None and len(can_read_documents) > 0:
                ...
                domain = [('id', '=', 0)]
        views = [(
            self.env.ref('document_management.general_document_action_document_quotation_part_file_kanban').id,
            'kanban'),
            (self.env.ref('document_management.view_document_quotation_file_form').id, 'form'),
            (self.env.ref('document_management.view_document_quotation_file_tree').id, 'list')]

        action = {"name": "Quotation Documents", "type": "ir.actions.act_window", "view_mode": "kanban,form,tree",
                  "view_type": "form", "context": {"create": False},
                  "res_model": "document.quotation.file", 'domain': domain, 'view_id': False,
                  'views': views}
        return action

    def call_back_parent_root(self):
        res = self.env['document.quotation.part'].sudo().browse(self.res_id)
        context = {'active_id': res.document_quotation_id.id}
        action = res.document_quotation_id.get_quotation_document_action_document_part(context)
        action['target'] = 'main'
        return action

    def call_back_parent_part(self, context=None):
        res = self.env['document.quotation.part'].sudo().browse(self.res_id)
        action = res.get_quotation_document_action_document_part_file(context)
        action['target'] = 'main'
        return action
    ...
    def create(self, vals):
        ...
        if vals.get('is_copy') == True:
            ...
            # find parent file id
            parent_file_id = self.env['document.quotation.part'].sudo().search([('id', '=', vals.get('res_id'))],
                                                                               limit=1).file_id
            template_url = False
            if file_type != 'folder':
                if file_type == 'doc':
        ...
        if file_type == 'doc':
            ...
            vals['google_drive_url'] = 'https://drive.google.com/drive/folders/' + google_drive_new_file[
                'id']
        # update write user
        # file=super(DocumentQuotationFile,self).create(vals)
        write_user_ids = []
        ...
        return super(DocumentQuotationFile, self).create(vals)

    def write(self, values):
        # update on google drive
        current_file = self.env['document.quotation.file'].sudo().browse(self.id)
        ...
        folder = super(DocumentQuotationFile, self).write(values)
        if values.get('name'):
            google_drive_helper = GoogleDriveHelper()
        ...
        if values.get('write_users') or values.get('read_users'):
            ...
            self.env.cr.execute(
                """select res_user_id from document_quotation_file_write_user_rel where res_file_id = %s""",
                (self.id,))
            ...
            self.env.cr.execute(
                """select res_user_id from document_quotation_file_read_user_rel where res_file_id = %s""",
                (self.id,))
        ...
        if 'public' in values:
            ...
            if not self.public:
                public_permissions = self.env['document.file.public.permission'].sudo().search(
                    [('model', '=', 'quotation'), ('file_id', '=', self.file_id)])
        ...
        for e in self:
            ...
            if e.public:
                self.env['document.file.public.permission'].sudo().search(
                    [('file_id', '=', e.file_id), ('model', '=', 'quotation')]).unlink()
        return super(DocumentQuotationFile, self).unlink()
