...
class DocumentProjectFile(models.Model):
    _name = "document.project.file"
    ...
    def _inverse_is_favorite(self):
        favorite_document = not_fav_document = self.env['document.project.file'].sudo()
        for document in self:
    ...
    type = fields.Selection(
        ...
        default='doc')
    # res_model = fields.Selection([('general', 'General'), ('project', 'Project'), ('quotation', 'Quotation')],
    ...
    write_users = fields.Many2many('res.users', 'document_project_file_write_user_rel', 'res_file_id',
                                   'res_user_id',
                                   string='Write Users', default=lambda self: self.env.user)
    read_users = fields.Many2many('res.users', 'document_project_file_read_user_rel', 'res_file_id',
                                  'res_user_id',
                                  string='Read Users')

    # can edit
    favorite_user_ids = fields.Many2many(
        'res.users', 'document_project_file_favorite_user_rel', 'document_id', 'user_id',
    ...
    current_permission_can_update = fields.Boolean(compute='_compute_current_permission_can_update', default=False,
                                                   store=False)
    # public permission
    public = fields.Boolean('Public', default=False, help='Check here for public document for anyone.',
                            track_visibility='onchange')
    public_type = fields.Selection([('read', 'Read'), ('write', 'Write'), ('comment', 'Comment')], default='read', track_visibility='onchange')
    # user can update
    ...
    def check_user_can_see_file(self):
        for rec in self:
            if len(rec.users_update) > 0:
                warning = []
                # users_can_see = [res.res_user_id for res in self.env['document.permission'].sudo().search([('res_id', '=', rec.res_id), ('model', '=', 'project')])]
                users_can_see = []
    ...
    @api.model
    def action_document_project_file_list(self):
        ...
        if self.user_has_groups('base.group_system'):
            ...
            self.env.cr.execute(
                """select id
from document_project_file
where id in (select res_file_id from document_project_file_write_user_rel where res_user_id = %s)
    or id in (select res_file_id from document_project_file_read_user_rel where res_user_id = %s)""",
                (self._uid, self._uid,))
            ...
            if can_read_documents is not None and len(can_read_documents) > 0:
                ...
                domain = [('id', '=', 0)]
        views = [(self.env.ref('document_management.general_document_action_document_project_part_file_kanban').id,
                  'kanban'),
                 (self.env.ref('document_management.view_document_project_file_form').id, 'form'),
                 (self.env.ref('document_management.view_document_project_file_tree').id, 'tree')]
        # action = self.env.ref('document_management.ir_actions_window_document_project_file_list_special').read()[0]
        action = {"name": "Project Documents", "type": "ir.actions.act_window", "view_mode": "kanban,form,tree",
                  "context": {"create": False},
                  "res_model": "document.project.file", 'domain': domain,
                  'views': views}
    ...
    def call_back_parent_root(self):
        res = self.env['document.project.part'].sudo().browse(self.res_id)
        context = {'active_id': res.document_project_id.id}
        action = res.document_project_id.get_project_document_action_document_part(context)
        action['target'] = 'main'
        return action

    def call_back_parent_part(self, context=None):
        res = self.env['document.project.part'].sudo().browse(self.res_id)
        action = res.get_project_document_action_document_part_file(context)
        action['target'] = 'main'
        return action
    ...
    def create(self, vals):
        ...
        # # find parent file id
        # parent_file_id = self.env['document.project.part'].sudo().search([('id', '=', vals.get('res_id'))],
        #                                                                  limit=1).file_id

        # get new parent folder and new template when copy
        ...
        return super(DocumentProjectFile, self).create(vals)

    def write(self, values):
        # update on google drive
        current_file = self.env['document.project.file'].sudo().browse(self.id)
        ...
        folder = super(DocumentProjectFile, self).write(values)
        if values.get('name'):
            google_drive_helper = GoogleDriveHelper()
        ...
        if values.get('write_users') or values.get('read_users'):
            ...
            self.env.cr.execute(
                """select res_user_id from document_project_file_write_user_rel where res_file_id = %s""",
                (self.id,))
            ...
            if all_write_user_ids is not None and len(all_write_user_ids) > 0:
                ...
                # self.env.cr.execute(
                #     """delete from document_permission where model like 'project' and type like 'read' and res_user_id in %s""",
                #     (tuple(e[0] for e in all_write_user_ids),))
                self.env.cr.execute(
            ...
            self.env.cr.execute(
                """select res_user_id from document_project_file_read_user_rel where res_file_id = %s""",
                (self.id,))
            ...
            if values.get('write_users'):
                ...
                body = "<p>Change Write Users: </p><p>" + 'Old Users: ' + (
                    old_write_users) + "</p><p>" + 'New Users: ' + (new_write_users) + '</p>'
            ...
            if values.get('read_users'):
                ...
                body = "<p>Change Reads Users: </p><p>" + 'Old Users: ' + (
                    old_read_users) + "</p><p>" + 'New Users: ' + (
                           new_read_users) + '</p>'
                self.message_post(body=body)
        # Tracking external Users
        # Handle in external user model
        if 'public' in values:
            if self.public:
            ...
            if not self.public:
                public_permissions = self.env['document.file.public.permission'].sudo().search(
                    [('model', '=', 'project'), ('file_id', '=', self.file_id)])
        ...
        for e in self:
            ...
            if e.public:
                self.env['document.file.public.permission'].sudo().search(
                    [('file_id', '=', e.file_id), ('model', '=', 'project')]).unlink()
        return super(DocumentProjectFile, self).unlink()
