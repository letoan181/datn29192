...
class ZK(object):
    ...
    def __send_command(self, command, command_string=b'', response_size=8):
        ...
        if command not in [const.CMD_CONNECT, const.CMD_AUTH] and not self.is_connect:
            raise UserError("instance are not connected.")
        ...
        try:
            if self.tcp:
                ...
                if self.__tcp_length == 0:
                    raise UserError("TCP packet invalid")
    ...
    def connect(self):
        ...
        # code goc cua no la const.CMD_ACK_UNAUTH (sh@dowWalker)
        if cmd_response.get('code') == const.CMD_ACK_UNAUTH:
            if self.verbose: a =("try auth")
        ...
        if cmd_response.get('status'):
            ...
            if cmd_response["code"] == const.CMD_ACK_UNAUTH:
                raise UserError("Unauthenticated")
            if self.verbose: a =("connect err response {} ".format(cmd_response["code"]))
            raise UserError("Invalid response: Can't connect")
    ...
    def disconnect(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't disconnect")
    ...
    def get_firmware_version(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("Can't read frimware version")
    ...
    def get_serialnumber(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("Can't read serial number")
    ...
    def get_platform(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("Can't read platform name")
    ...
    def get_mac(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't read mac address")
    ...
    def get_fp_version(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't read fingerprint version")
    ...
    def get_pin_width(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can0t get pin width")
    ...
    def free_data(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't free data")
    ...
    def read_sizes(self):
        ...
        if cmd_response.get('status'):
            if self.verbose: a =(codecs.encode(self.__data, 'hex'))
            ...
            raise UserError("can't read sizes")
    ...
    def unlock(self, time=3):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("Can't open door")
    ...
    def restart(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't restart device")
    ...
    def get_time(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't get time")
    ...
    def set_time(self, timestamp):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't set time")
    ...
    def poweroff(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't poweroff")
    ...
    def refresh_data(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't refresh data")
    ...
    def set_user(self, uid=None, name='', privilege=0, password='', group_id='', user_id='', card=0):
        ...
        if self.user_packet_size == 28:  # self.firmware == 6:
            ...
            try:
                ...
                if self.verbose: a =("s_h Error pack: %s" % e)
                if self.verbose: a =("Error pack: %s" % sys.exc_info()[0])
                raise UserError("Can't pack user")
        else:
        ...
        cmd_response = self.__send_command(command, command_string, response_size)
        if self.verbose: a =("Response: %s" % cmd_response)
        if not cmd_response.get('status'):
            raise UserError("Can't set user")
        self.refresh_data()
    ...
    def save_user_template(self, user, fingers=[]):
        ...
        if not isinstance(user, User):
            ...
            if len(tusers) == 1:
                ...
                if len(tusers) == 1:
                    ...
                    raise UserError("Can't find user")
        ...
        cmd_response = self.__send_command(command, command_string)
        if not cmd_response.get('status'):
            raise UserError("Can't save utemp")
        self.refresh_data()
    ...
    def _send_with_buffer(self, buffer):
        ...
        cmd_response = self.__send_command(command, command_string)
        if not cmd_response.get('status'):
            raise UserError("Can't prepare data")
    ...
    def __send_chunk(self, command_string):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("Can't send chunk")
    ...
    def delete_user(self, uid=0, user_id=''):
        ...
        cmd_response = self.__send_command(command, command_string)
        if not cmd_response.get('status'):
            raise UserError("Can't delete user")
        self.refresh_data()
    ...
    def get_user_template(self, uid, temp_id=0, user_id=''):
        ...
        for _retries in range(3):
            ...
            if self.verbose: a =("retry get_user_template")
        else:
            if self.verbose: a =("Can't read/find finger")
            return None
    ...
    def get_templates(self):
        ...
        if size < 4:
            if self.verbose: a =("WRN: no user data")
            return []
        total_size = unpack('i', templatedata[0:4])[0]
        if self.verbose: a =("get template total size {}, size {} len {}".format(total_size, size, len(templatedata)))
    ...
    def get_users(self):
        ...
        if self.verbose: a =("user size {} (= {})".format(size, len(userdata)))
        if size <= 4:
            a =("WRN: missing user data")
            return []
        total_size = unpack("I", userdata[:4])[0]
        self.user_packet_size = total_size / self.users
        if not self.user_packet_size in [28, 72]:
            if self.verbose: a =("WRN packet size would be  %i" % self.user_packet_size)
        userdata = userdata[4:]
        if self.user_packet_size == 28:
            while len(userdata) >= 28:
                ...
                users.append(user)
                if self.verbose: a =("[6]user:", uid, privilege, password, name, card, group_id, timezone, user_id)
    ...
    def reg_event(self, flags):
        ...
        cmd_response = self.__send_command(command, command_string)
        if not cmd_response.get('status'):
            raise UserError("cant' reg events %i" % flags)
    ...
    def enroll_user(self, uid=0, temp_id=0, user_id=''):
        ...
        cmd_response = self.__send_command(command, command_string)
        if not cmd_response.get('status'):
            raise UserError("Cant Enroll user #%i [%i]" % (uid, temp_id))
        self.__sock.settimeout(60)
        attempts = 3
        while attempts:
            if self.verbose: a =("A:%i esperando primer regevent" % attempts)
            data_recv = self.__sock.recv(1032)
            self.__ack_ok()
            if self.verbose: a =(codecs.encode(data_recv, 'hex'))
            if self.tcp:
                if len(data_recv) > 16:
                    res = unpack("H", data_recv.ljust(24, b"\x00")[16:18])[0]
                    if self.verbose: a =("res %i" % res)
                    if res == 0 or res == 6 or res == 4:
                        if self.verbose: a =("posible timeout  o reg Fallido")
                        break
            else:
                if len(data_recv) > 8:
                    ...
                    if self.verbose: a =("res %i" % res)
                    if res == 6 or res == 4:
                        if self.verbose: a =("posible timeout")
                        break
            if self.verbose: a =("A:%i esperando 2do regevent" % attempts)
            data_recv = self.__sock.recv(1032)
            self.__ack_ok()
            if self.verbose: a =(codecs.encode(data_recv, 'hex'))
            if self.tcp:
        ...
        while not self.end_live_capture:
            try:
                ...
                if not header[0] == const.CMD_REG_EVENT:
                    if self.verbose: a =("not event! %x" % header[0])
                    continue
                if not len(data):
                    if self.verbose: a =("empty")
                    continue
                while len(data) >= 12:
                    if len(data) == 12:
                ...
                if self.verbose: a =("time out")
                yield None  # return to keep watching
            except (KeyboardInterrupt, SystemExit):
                if self.verbose: a =("break")
                break
        if self.verbose: a =("exit gracefully")
        self.__sock.settimeout(self.__timeout)
        self.reg_event(0)
        if not was_enabled:
            self.disable_device()
    ...
    def clear_data(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("can't clear data")
    ...
    def __recieve_tcp_data(self, data_recv, size):
        ...
        if self.verbose: a =("tcp_length {}, size {}".format(tcp_length, size))
        if tcp_length <= 0:
            if self.verbose: a =("Incorrect tcp packet")
            return None, b""
        if (tcp_length - 8) < size:
            if self.verbose: a =("tcp length too small... retrying")
            resp, bh = self.__recieve_tcp_data(data_recv, tcp_length - 8)
            data.append(resp)
            size -= len(resp)
            if self.verbose: a =("new tcp DATA packet to fill misssing {}".format(size))
            data_recv = bh + self.__sock.recv(size + 16)
            if self.verbose: a =("new tcp DATA starting with {} bytes".format(len(data_recv)))
            resp, bh = self.__recieve_tcp_data(data_recv, size)
            data.append(resp)
            if self.verbose: a =("for misssing {} recieved {} with extra {}".format(size, len(resp), len(bh)))
            return b''.join(data), bh
        recieved = len(data_recv)
        if self.verbose: a =("recieved {}, size {}".format(recieved, size))
        response = unpack('HHHH', data_recv[8:16])[0]
        if recieved >= (size + 32):
            if response == const.CMD_DATA:
                resp = data_recv[16: size + 16]
                if self.verbose: a =("resp complete len {}".format(len(resp)))
                return resp, data_recv[size + 16:]
            else:
                if self.verbose: a =("incorrect response!!! {}".format(response))
                return None, b""
        else:
            if self.verbose: a =("try DATA incomplete (actual valid {})".format(recieved - 16))
            data.append(data_recv[16: size + 16])
            size -= recieved - 16
            broken_header = b""
            if size < 0:
                broken_header = data_recv[size:]
                if self.verbose: a =("broken", (broken_header).encode('hex'))
            if size > 0:
    ...
    def __recieve_raw_data(self, size):
        """ partial data ? """
        data = []
        if self.verbose: a =("expecting {} bytes raw data".format(size))
        while size > 0:
            data_recv = self.__sock.recv(size)
            recieved = len(data_recv)
            if self.verbose: a =("partial recv {}".format(recieved))
            if recieved < 100 and self.verbose: a =("   recv {}".format(codecs.encode(data_recv, 'hex')))
            data.append(data_recv)
            size -= recieved
            if self.verbose: a =("still need {}".format(size))
        return b''.join(data)
        ...
        if self.__response == const.CMD_DATA:
            if self.tcp:
                ...
                if len(self.__data) < (self.__tcp_length - 8):
                    ...
                    if self.verbose: a =("need more data: {}".format(need))
                    more_data = self.__recieve_raw_data(need)
                    return b''.join([self.__data, more_data])
                else:
                    if self.verbose: a =("Enough data")
                    return self.__data
            else:
                if self.verbose: a =("_rc len is {}".format(len(self.__data)))
                return self.__data
        elif self.__response == const.CMD_PREPARE_DATA:
            data = []
            size = self.__get_data_size()
            if self.verbose: a =("recieve chunk: prepare data size is {}".format(size))
            if self.tcp:
                ...
                if len(data_recv) < 16:
                    a =("trying to complete broken ACK %s /16" % len(data_recv))
                    if self.verbose: a =(data_recv.encode('hex'))
                    data_recv += self.__sock.recv(16 - len(data_recv))  # TODO: CHECK HERE_!
                if not self.__test_tcp_top(data_recv):
                    if self.verbose: a =("invalid chunk tcp ACK OK")
                    return None
                response = unpack('HHHH', data_recv[8:16])[0]
                if response == const.CMD_ACK_OK:
                    if self.verbose: a =("chunk tcp ACK OK!")
                    return b''.join(data)
                if self.verbose: a =("bad response %s" % data_recv)
                if self.verbose: a =(codecs.encode(data, 'hex'))
                return None
            ...
            while True:
                ...
                if self.verbose: a =("# packet response is: {}".format(response))
                if response == const.CMD_DATA:
                    ...
                    if self.verbose: a =("broken!")
                    break
                if self.verbose: a =("still needs %s" % size)
            return b''.join(data)
        else:
            if self.verbose: a =("invalid response %s" % self.__response)
            return None
    ...
    def __read_chunk(self, start, size):
        ...
        for _retries in range(3):
            ...
            raise UserError("can't read chunk %i:[%i]" % (start, size))
    ...
    def read_with_buffer(self, command, fct=0, ext=0):
        ...
        if self.verbose: a =("rwb cs", command_string)
        response_size = 1024
        data = []
        start = 0
        cmd_response = self.__send_command(1503, command_string, response_size)
        if not cmd_response.get('status'):
            raise UserError("RWB Not supported")
        ...
        if self.verbose: a =("size fill be %i" % size)
        remain = size % MAX_CHUNK
        ...
        if remain:
            ...
            start += remain
        self.free_data()
        if self.verbose: a =("_read w/chunk %i bytes" % start)
    ...
    def get_attendance(self):
        ...
        if record_size == 8:
            while len(attendance_data) >= 8:
                ...
                if self.verbose: a =(codecs.encode(attendance_data[:8], 'hex'))
            ...
            while len(attendance_data) >= 16:
                ...
                user_id = str(user_id)
                if self.verbose: a =(codecs.encode(attendance_data[:16], 'hex'))
                attendance_data = attendance_data[16:]
                tuser = list(filter(lambda x: x.user_id == user_id, users))
                if not tuser:
                    if self.verbose: a =("no uid {}", user_id)
                    uid = str(user_id)
            ...
            while len(attendance_data) >= 40:
                ...
                if self.verbose: a =(codecs.encode(attendance_data[:40], 'hex'))
                user_id = (user_id.split(b'\x00')[0]).decode(errors='ignore')
                timestamp = self.__decode_time(timestamp)
    ...
    def clear_attendance(self):
        ...
        if cmd_response.get('status'):
            ...
            raise UserError("Can't clear response")
